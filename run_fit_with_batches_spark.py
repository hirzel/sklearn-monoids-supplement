import argparse
from config import n_folds, n_batches_per_fold, get_large_data, get_pipeline, get_pipeline_spark, get_pipeline_sklearn, get_pipeline_sklearn_rasl, regression_datasets, classification_datasets, get_scorer
import time
import numpy as np
import os
import math
...
def run_fit_with_batches(split):
    ...
    if args.expt_type == 0:
        ...
        batching = Batching(operator=pipeline, batch_size=args.batch_size, max_resident=128000000000, inmemory=True)
        t1 = time.time()
...
#        print(f"f1:{f1}", f1)
        ...
        pipeline = get_pipeline_spark(args.dataset)
