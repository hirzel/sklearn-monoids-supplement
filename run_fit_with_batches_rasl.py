...
def run_fit_with_batches(split):
    ...
    if args.expt_type == 0:
        ...
        print("3HERE!!!")
        batching = Batching(operator=pipeline, batch_size=args.batch_size, max_resident=args.max_resident, inmemory=True)
        t1 = time.time()
        ...
        trained = fit_with_batches(pipeline, train_X, None, None, np.unique(test_y),
                                           args.max_resident, PrioResourceAware(), partial_transform=False, verbose=2, progress_callback=None)
        t2 = time.time()
...
#        print(f"f1:{f1}", f1)
        ...
        t2 = time.time()
        #print(trained_pipeline.transform(test_X))
        #import pdb;pdb.set_trace()
        print("Training complete")
    ...
    elif args.expt_type==3:
        print("rasl fit to be started")
        (train_X, train_y), (test_X, test_y) = get_data(args.dataset, astype="pandas")
